...
void MapUI::PrintMenu() {
  ...
  case '3':
  {
    ...
    results = map.CalculateShortestPath_BFS(input1, input2);
  ...
  case '4':
  {
    ...
    std::cout << menu;
    std::cout << "Time taken by function: " << duration.count() << " us" << std::endl << std::endl;
    ...
    std::cout << "Calculating ..." << std::endl;
    start = std::chrono::high_resolution_clock::now();
    results = map.TravellingTrojan_GeneticAlgo(locations);
    stop = std::chrono::high_resolution_clock::now();
    duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
    CreateAnimation(results.second, "output0_genetic_algo.avi");
    menu = "*************************Results******************************\n";
    std::cout << menu;
    menu = "TravellingTrojan_Genetic_Algo\n";
    std::cout << menu;
    if (results.second.size() != 0) {
      for (auto x : results.second[results.second.size()-1]) std::cout << "\"" << x << "\",";
      std::cout << "\nThe distance of the path is:" << results.first << " miles" << std::endl;
    ...
    menu = "**************************************************************\n"
           "You could find your animation at src/lib/output0_genetic_algo.avi.\n";
    std::cout << menu;
    std::cout << "Time taken by function: " << duration.count()/1000 << " ms" << std::endl << std::endl;
    PrintMenu();
    break;
